...
/**
	...
	* @since             1.0
	* @package           hw-default-payment-gateway-for-woocommerce
	*
	* @wordpress-plugin
	* Plugin Name:       	Default Payment Gateway for WooCommerce
	...
	* Text Domain:       	hw-default-payment-gateway-for-woocommerce
...
function hw_wc_default_gateway_load_scripts() {
  wp_enqueue_style('hw-woocommerce-default-gateway', HW_WC_DEFAULT_GATEWAY_PLUGIN_URL . '/assets/css/hw-woocommerce-default-gateway.css', null, '1.5', 'all');
}
...
/* instant save after check new value --> send value to server side */
...
check_ajax_referer( 'hw-woocommerce-default-gateway', 'security' );
	if (isset($_POST['chosen_gateway'])) {
		...
		echo wp_json_encode(array('success'=>$save_option, 'saved_gateway'=>$chosen_gateway));
	} else {
		echo wp_json_encode(array('error'=>'chosen_gateway is missing', 'data'=>filter_input_array($_POST, FILTER_SANITIZE_FULL_SPECIAL_CHARS)));
	}
...
add_filter( 'plugin_action_links_hw-default-payment-gateway-for-woocommerce/hw-default-payment-gateway-for-woocommerce.php', 'hw_woocommerce_default_gateway_settings_link' );
